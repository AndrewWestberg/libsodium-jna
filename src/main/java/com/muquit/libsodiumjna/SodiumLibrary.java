...
public class SodiumLibrary {
    ...
    public interface Sodium extends Library {
        ...
        // -- Sizes of various value types involved in the VRF calculations.
        int crypto_vrf_proofbytes();

        int crypto_vrf_publickeybytes();

        int crypto_vrf_secretkeybytes();

        int crypto_vrf_seedbytes();

        int crypto_vrf_outputbytes();

        int crypto_vrf_keypair(byte[] vrfVkey, byte[] vrfSkey);

        int crypto_vrf_keypair_from_seed(byte[] vrfVkey, byte[] vrfSkey, byte[] seed);

        // For some reason, these two fail. We can just do them on the java side though
        // since they are simple array copies.
        // int crypto_vrf_sk_to_pk(byte[] vrfVkey, byte[] vrfSkey);
        // int crypto_vrf_sk_to_seed(byte[] seed, byte[] vrfSkey);

        int crypto_vrf_prove(byte[] proof, byte[] vrfSkey, byte[] m, long mlen);
        int crypto_vrf_verify(
                byte[] output,
		      byte[] vrfVkey,
		      byte[] proof,
		      byte[] m,
...
//        logger.debug(">>> alg: " +  sodium().crypto_pwhash_alg_argon2i13());
    ...
    */
    ...
    /**
     * Generate a random seed for usage with VRF
     *
     * @return random seed
     * @throws SodiumLibraryException
     */
    public static byte[] cryptoVrfRandomSeed() throws SodiumLibraryException {
        int seedBytes = sodium().crypto_vrf_seedbytes();
        byte[] seed = new byte[seedBytes];
        sodium().randombytes_buf(seed, seedBytes);
        return seed;
    }
     ...
     * @throws SodiumLibraryException
     */
    public static SodiumKeyPair cryptoVrfKeypair() throws SodiumLibraryException {
        byte[] vrfVkey = new byte[sodium().crypto_vrf_publickeybytes()];
        byte[] vrfSkey = new byte[sodium().crypto_vrf_secretkeybytes()];
        int rc = sodium().crypto_vrf_keypair(vrfVkey, vrfSkey);
        if (rc != 0) {
            throw new SodiumLibraryException("libsodium crypto_vrf_keypair() failed, returned " + rc + ", expected 0");
        }
        return new SodiumKeyPair(vrfVkey, vrfSkey);
    }

    /**
     * Generate a VRF keypair based on a seed value
     *
     * @throws SodiumLibraryException
     */
    public static SodiumKeyPair cryptoVrfKeypairFromSeed(byte[] seed) throws SodiumLibraryException {
        byte[] vrfVkey = new byte[sodium().crypto_vrf_publickeybytes()];
        byte[] vrfSkey = new byte[sodium().crypto_vrf_secretkeybytes()];
        int rc = sodium().crypto_vrf_keypair_from_seed(vrfVkey, vrfSkey, seed);
        if (rc != 0) {
            throw new SodiumLibraryException("libsodium crypto_vrf_keypair_from_seed() failed, returned " + rc + ", expected 0");
        }
        return new SodiumKeyPair(vrfVkey, vrfSkey);
    }

    /**
     * Retrieve the public vrf vkey based on a secret vrf key
     */
    public static byte[] cryptoVrfSkeyToVkey(byte[] vrfSkey) throws SodiumLibraryException {
        return Arrays.copyOfRange(vrfSkey, 32, 64);
    }

    /**
     * Retrieve the public vrf vkey based on a secret vrf key
     */
    public static byte[] cryptoVrfSkeyToSeed(byte[] vrfSkey) throws SodiumLibraryException {
        return Arrays.copyOfRange(vrfSkey, 0, 32);
    }

    public static byte[] cryptoVrfProve(byte[] vrfSkey, byte[] m) throws SodiumLibraryException {
        byte[] proof = new byte[sodium().crypto_vrf_proofbytes()];
        int rc = sodium().crypto_vrf_prove(proof, vrfSkey, m, m.length);
        if (rc != 0) {
            throw new SodiumLibraryException("libsodium crypto_vrf_prove() failed, returned " + rc + ", expected 0");
        }
        return proof;
    }

    public static byte[] cryptoVrfVerify(byte[] vrfVkey, byte[] proof, byte[] m) throws SodiumLibraryException {
        byte[] output = new byte[sodium().crypto_vrf_outputbytes()];
        int rc = sodium().crypto_vrf_verify(output, vrfVkey, proof, m, m.length);
        if (rc != 0) {
            throw new SodiumLibraryException("libsodium crypto_vrf_verify() failed, returned " + rc + ", expected 0");
        }
        return output;
    }

    public static byte[] cryptoVrfProofToHash(byte[] proof) throws SodiumLibraryException {
