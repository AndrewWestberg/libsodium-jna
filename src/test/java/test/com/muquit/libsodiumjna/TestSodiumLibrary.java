...
public class TestSodiumLibrary {
	...
	*/
     ...
     *
     * @throws SodiumLibraryException <p>
    ...
    @Test
    public void testVrfKeypair() throws SodiumLibraryException {
        SodiumKeyPair vrfKeypair = SodiumLibrary.cryptoVrfKeypair();
        byte[] publicKey = vrfKeypair.getPublicKey();
        byte[] privateKey = vrfKeypair.getPrivateKey();
        String hexPublicKey = SodiumUtils.binary2Hex(publicKey);
        String hexPrivateKey = SodiumUtils.binary2Hex(privateKey);
        logger.info("Generated Public VRF key " + publicKey.length + " bytes");
        logger.info(hexPublicKey);
        logger.info("Generated Private VRF key " + privateKey.length + " bytes");
        logger.info(hexPrivateKey);
    }

    @Test
    public void testVrfKeypairFromSeed() throws SodiumLibraryException {
        byte[] seed = SodiumLibrary.cryptoVrfRandomSeed();
        String hexSeed = SodiumUtils.binary2Hex(seed);
        logger.info("Generated random seed " + seed.length + " bytes");
        logger.info(hexSeed);
        SodiumKeyPair vrfKeypair = SodiumLibrary.cryptoVrfKeypairFromSeed(seed);
        byte[] publicKey = vrfKeypair.getPublicKey();
        byte[] privateKey = vrfKeypair.getPrivateKey();
        String hexPublicKey = SodiumUtils.binary2Hex(publicKey);
        String hexPrivateKey = SodiumUtils.binary2Hex(privateKey);
        ...
        // Should get the exact same keys based on the same seed.
        SodiumKeyPair vrfKeypair2 = SodiumLibrary.cryptoVrfKeypairFromSeed(seed);
        assertArrayEquals(publicKey, vrfKeypair2.getPublicKey());
        assertArrayEquals(privateKey, vrfKeypair2.getPrivateKey());
    }

    @Test
    public void testVrfSkeyToVkey() throws SodiumLibraryException {
        byte[] seed = SodiumLibrary.cryptoVrfRandomSeed();
        Arrays.fill(seed, (byte) 0);
        String hexSeed = SodiumUtils.binary2Hex(seed);
        logger.info("Seed " + seed.length + " bytes");
        logger.info(hexSeed);
        SodiumKeyPair vrfKeypair = SodiumLibrary.cryptoVrfKeypairFromSeed(seed);
        byte[] publicKey = vrfKeypair.getPublicKey();
        byte[] privateKey = vrfKeypair.getPrivateKey();
        ...
        byte[] publicKey2 = SodiumLibrary.cryptoVrfSkeyToVkey(privateKey);
        assertArrayEquals(publicKey, publicKey2);
    }

    @Test
    public void testVrfSkeyToSeed() throws SodiumLibraryException {
        byte[] seed = SodiumLibrary.cryptoVrfRandomSeed();
        Arrays.fill(seed, (byte) 0);
        String hexSeed = SodiumUtils.binary2Hex(seed);
        logger.info("Seed " + seed.length + " bytes");
        ...
        byte[] seed2 = SodiumLibrary.cryptoVrfSkeyToSeed(privateKey);
        assertArrayEquals(seed, seed2);
    }

    @Test
    public void testVrfProveAndVerify() throws SodiumLibraryException {
        byte[] seed = SodiumLibrary.cryptoVrfRandomSeed();
        String hexSeed = SodiumUtils.binary2Hex(seed);
        logger.info("Seed " + seed.length + " bytes");
        logger.info(hexSeed);
        ...
        byte[] message = "prove it!".getBytes(StandardCharsets.UTF_8);

        byte[] proof = SodiumLibrary.cryptoVrfProve(privateKey, message);
        String proofHex = SodiumUtils.binary2Hex(proof);
        logger.info("Proof " + proof.length + " bytes");
        logger.info(proofHex);

        byte[] verify = SodiumLibrary.cryptoVrfVerify(publicKey, proof, message);
        String verifyHex = SodiumUtils.binary2Hex(verify);
        logger.info("Verify " + verify.length + " bytes");
        logger.info(verifyHex);
    }

    @Test
    public void testVrfProofToHash() throws SodiumLibraryException {
        byte[] seed = SodiumLibrary.cryptoVrfRandomSeed();
        String hexSeed = SodiumUtils.binary2Hex(seed);
        logger.info("Seed " + seed.length + " bytes");
        logger.info(hexSeed);
        ...
        byte[] hash = SodiumLibrary.cryptoVrfProofToHash(proof);
        String hashHex = SodiumUtils.binary2Hex(hash);
        logger.info("Hash " + hash.length + " bytes");
        logger.info(hashHex);
    }
